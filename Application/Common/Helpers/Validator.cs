...
public static class Validator
{
    ...
    public static bool IsValid(this Client guest)
    => !string.IsNullOrEmpty(guest.FullName)
       && guest.FullName.Length >= 2
       && guest.FullName.Length <= 50;

    public static bool IsExsit(this OnlineBron onlineBron, IEnumerable<OnlineBron> onlineBrons)
        => onlineBron != null
        && onlineBrons.Any(ob => ob.StartDate == onlineBron.StartDate);

    public static bool IsExsit(this Client client, IEnumerable<Client> clients)
        => client != null
        && clients.Any(c => c.FullName == client.FullName
        && c.RoomId == client.RoomId
        && c.PersonCount == client.PersonCount);
}
